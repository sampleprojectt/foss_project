...
(open(playlist_vid_urls_file,"w")).close()
with open(playlist_vid_urls_file,"a") as f: 
	# Get URL
	...
	num_non_private_videos=len(json_response["items"])
	# Write video URL's in the first response to file
	n=0
	for i in range(0,num_non_private_videos):
		n=n+1
		f.write("https://www.youtube.com/watch?v=")
		f.write(json_response["items"][i]["snippet"]["resourceId"]["videoId"])
		f.write("\n")

	number_of_pages=int((json_response["pageInfo"]["totalResults"]/maxresults_per_page))

	# Get details of next videos (51st video,52nd video.. and so on if exists) 
	for i in range(1,number_of_pages):
		req_param["pageToken"]=json_response["nextPageToken"]
		response=requests.get("https://www.googleapis.com/youtube/v3/playlistItems",params=req_param)
		json_response=response.json()
		num_non_private_videos=len(json_response["items"])
		for j in range(0,num_non_private_videos):
			n=n+1
			f.write("https://www.youtube.com/watch?v=")
			f.write(json_response["items"][j]["snippet"]["resourceId"]["videoId"])
			f.write("\n")

	# Get details of videos in the last response page
